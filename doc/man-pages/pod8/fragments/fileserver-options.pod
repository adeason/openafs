...
=item B<-rxl> <I<number of rx listener threads>>

Sets the number of threads to run that are used for receiving packets from the
wire. Raising this can improve performance for heavily-loaded fileservers on
machines with multiple CPU cores.

Note that these threads are unrelated to the processes specified by B<-p>. The
threads specified by B<-rxl> are created in addition to those created for
B<-p>.

You must specify a number between C<1> and C<1024>, inclusive. The default is
C<4>; if B<-S> is given, then the default is C<1>.
