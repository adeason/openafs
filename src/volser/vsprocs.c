...
/**
 * @brief   Return a locked, network-order VLDB entry
 *
 * @param[in]   volid	    volid of the desired volume entry
 * @param[in]   voltype	    volume type of the desired volume entry (e.g.,
 *			    RWVOL), or -1
 * @param[in]   vlop	    VLOP_* code for the operation requesting to update
 *			    the entry
 ...
 */
static afs_int32
GetLockedEntry(afs_uint32 volid, afs_int32 voltype, int vlop,
	       struct nvldbentry *vlentry)
{
    ...
    code = VLDB_GetEntryByID(volid, voltype, vlentry);
    if (code != 0) {
	afs_int32 savecode = code;

	code = ubik_VL_ReleaseLock(cstruct, 0, volid, voltype,
				   LOCKREL_OPCODE | LOCKREL_AFSID |
				   LOCKREL_TIMESTAMP);
	if (code != 0) {
	    EPRINT1(code,
		   "Could not release lock on VLDB entry for volume %lu",
		    (unsigned long)volid);
	}
...
#ifdef AFS_RXGK_ENV
    if (uvindex == RX_SECIDX_GK && !uv_rxgk_novldb) {
...
#ifdef AFS_RXGK_ENV
static int
get_rxgk_combined_sc(afs_uint32 aserver, struct rx_securityClass **a_sc)
{
    static struct rx_securityClass *last_sc;
    static afs_uint32 last_server;
	...
	*a_sc = last_sc;
    ...
    /*
     * Remember the last sc we generated and reuse it for the same server, to
     * slightly avoid hammering the VLDB to create effectively the same sc when
     * contacting the same server repeatedly. This is a very naive cache just
     * to avoid the worst case for now, since it's so easy to do. This can be
     * replaced by a more complex solution in the future if that ever seems
     * warranted.
     */
    last_server = aserver;
    if (last_sc != NULL) {
...
UV_BindVolser(afs_uint32 aserver, struct rx_connection **a_conn)
{
    ...
    if (uvindex == RX_SECIDX_GK && !uv_rxgk_novldb) {
	afs_int32 code;
	code = get_rxgk_combined_sc(aserver, &sc);
	if (code != 0) {
	    return code;
	}
    }
	       ...
	       afs_uint32 atoserver, afs_int32 atopart, int flags)
{
    ...
    vcode = GetLockedEntry(afromvol, RWVOL, VLOP_MOVE, &entry);
    EGOTO1(mfail, vcode, "Could not fetch locked entry for volume %u \n", afromvol);
    islocked = 1;
...
UV_BackupVolume(afs_uint32 aserver, afs_int32 apart, afs_uint32 avolid)
{
	...
	code = GetLockedEntry(avolid, RWVOL, VLOP_BACKUP, &entry);
	if (code) {
	    fprintf(STDERR,
		    "Could not fetch locked VLDB entry for volume %lu\n",
		    (unsigned long)avolid);
...
CheckVldb(struct nvldbentry * entry, afs_int32 * modified, afs_int32 * deleted)
{
  ...
  retry:
     ...
     */
    if (++pass == 2) {
	code = GetLockedEntry(entry->volumeId[RWVOL], RWVOL,
			      VLOP_DELETE, entry);
	if (code) {
...
UV_RenameVolume(struct nvldbentry *entry, char oldname[], char newname[])
{
    ...
    /*
     * Get the entry again (under lock) and verify the volume hasn't otherwise
     * changed.  We use VLOP_ADDSITE due to the lack of a specific define for
     * renames.
     */
    vcode = GetLockedEntry(entry->volumeId[RWVOL], RWVOL, VLOP_ADDSITE,
			   &storeEntry);
    if (vcode) {
	fprintf(STDERR,
		" Could not fetch locked VLDB entry for volume %u \n",
