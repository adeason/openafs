...
static int
ctlcall_freeze_install(struct ubik_freeze_client *freeze,
		       struct ubik_version64 *old_vers,
		       struct ubik_version64 *new_vers, char *new_suffix,
		       char *backup_suffix)
{
    struct afsctl_req_inargs inargs;
    ...
    inargs.op = UFRZCTL_OP_INSTALL;
    inargs.u.ufrz_inst.freezeid = freeze->freezeid;
    inargs.u.ufrz_inst.old_version = *old_vers;
    inargs.u.ufrz_inst.new_version = *new_vers;
    inargs.u.ufrz_inst.new_suffix = new_suffix;
    inargs.u.ufrz_inst.backup_suffix = backup_suffix;

    return afsctl_client_call(&freeze->ctl_cinfo, &inargs, NULL);
}

static int
ctlcall_freeze_dist(struct ubik_freeze_client *freeze)
{
    struct afsctl_req_inargs inargs;

    inargs.op = UFRZCTL_OP_DIST;
    inargs.u.ufrz_dist_freezeid = freeze->freezeid;
		 ...
		 struct ubik_version64 *a_version, char **a_dbpath)
{
    ...
    if (freeze->need_rw) {
	freeze_args.readwrite = 1;
    } else {
	freeze_args.readonly = 1;
    }
...
calc_suffix(struct ubik_freeze_client *freeze, char *new_path, char **a_suffix)
{
    ...
    if (new_len > base_len && strncmp(abs_base, new_path, base_len) == 0) {
	*a_suffix = strdup(&new_path[base_len]);
	if (*a_suffix == NULL) {
	    code = UNOMEM;
	    goto done;
	}
		   ...
		   char *backup_suffix)
{
    ...
    code = ctlcall_freeze_install(freeze, &freeze->db_vers, &version, suffix,
				  backup_suffix);
    if (code != 0) {
	goto done;
    }
...
{
    return ctlcall_freeze_dist(freeze);
}
