...
cb_stateRestoreFEHash(struct fs_dump_state * state)
{
	...
	ViceLog(0, ("cb_stateRestoreFEHash: invalid cb_fehash_hdr magic\n"));
	ret = 1;
	goto done;
    }
	...
	ViceLog(0, ("cb_stateRestoreFEHash:  expected len %d != actual len %d + %d\n",
		    state->cb_fehash_hdr->len, (int) sizeof(struct callback_state_fehash_header), len));
	ret = 1;
	goto done;
    }
