...
afs_uint32 * HashTable;	    /* File entry hash table */
afs_int32 FEhashsize;	    /* number of buckets in HashTable */
...
InitCallBack(int nblks)
{
	 ...
	 * what we get by default with the '-L' fileserver switch). Figure out
	 * a hashtable size so our hash chains are around FE_CHAIN_TARGET long.
	 * Start at the old historical hashtable size (FEHASH_SIZE_OLD), and
	 * count up until we get a reasonable number.
	 */
	int safety;
	static const int FE_CHAIN_TARGET = 16;
	workingHashSize = FEHASH_SIZE_OLD;
	for (safety = 0; nblks / workingHashSize > FE_CHAIN_TARGET; safety++) {
	    workingHashSize *= 2;
	    opr_Assert(safety < 100);
	}
...
cb_stateRestoreTimeouts(struct fs_dump_state * state)
{
	...
	ViceLog(0, ("cb_stateRestoreTimeouts: header len %d != %d\n",
		state->cb_timeout_hdr->len,
		(int)(sizeof(struct callback_state_timeout_header) + len)));
	ret = 1;
	goto done;
    }
