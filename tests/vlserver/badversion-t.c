...
runtest(struct testinfo *test)
{
    ...
    code = afstest_StartServer(&afstest_server_vl, dirname, &pid);
    if (code != 0) {
	afs_com_err(argv0, code, "while starting vlserver");
	goto done;
    }
	...
	if (code != 0) {
	    afs_com_err(argv0, code, "while stopping vlserver");
	}
    }
    return code;
}
...
main(int argc, char **argv)
{
    ...
    if (code != 0) {
	afs_com_err(argv0, code, "while building security class");
	code = 1;
	goto done;
    }
