...
static int
buf_cmp_nodiag(struct rx_opaque *buf_a, struct rx_opaque *buf_b)
{
    ...
    opr_Assert(buf_a->len > buf_b->len);
    return 1;
}
...
{
    int diff = buf_cmp_nodiag(buf_a, buf_b);
    if (diff != target_diff) {
...
check_all_items(struct okv_trans *tx)
{
	...
	for (item = data_items; item->key.val != NULL; item++) {
	    if (buf_cmp_nodiag(&item->key, &key) == 0) {
		found_item = item;
		break;
	    }
	}
...
check_dbh(struct okv_dbhandle *dbh, int threaded)
{
    ...
    is_int(5, (st.os_entries != NULL ? *st.os_entries : -1),
    ...
    is_int(3, (st.os_entries != NULL ? *st.os_entries : -1),
...
test_existing(void)
{
    ...
    if (dir_path == NULL) {
	skip_block(362, "%s does not exist for this platform", name);
	goto done;
    }
